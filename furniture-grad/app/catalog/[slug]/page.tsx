...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-5 py-10 max-md:pb-24">
        ...
        <div className="grid md:grid-cols-2 gap-0">
          ...
          <div className="p-10 flex flex-col justify-center max-md:p-6 min-w-0">
            ...
            <div className="max-md:hidden flex gap-3 items-center">
              <ContactSellerButton />
              <FavoriteButton productId={product.id} className="w-14 h-14 shrink-0" />
            </div>
          </div>
        </div>
      </div>

      {/* Мобильная закреплённая панель "Связаться" — над нижним меню сайта */}
      <div
        className="md:hidden fixed left-0 right-0 z-[500] bg-white border-t border-[#eee] p-3"
