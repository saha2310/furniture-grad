...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      {/* pb-24 на мобиле: место под фиксированную кнопку "Связаться" внизу
          экрана, чтобы она не перекрывала последний блок страницы. На
          десктопе фиксированной панели нет (кнопка встроена в блок товара),
          поэтому там достаточно обычного отступа. */}
      <div className="max-w-[1400px] mx-auto px-5 py-10 pb-24 md:pb-10">
        <Breadcrumbs
          items={[
            { label: 'Главная', href: '/' },
            { label: 'Каталог', href: '/catalog' },
            ...(product.categories
              ? [{ label: product.categories.name, href: `/catalog?category=${product.categories.id}` }]
              : []),
            { label: product.name },
          ]}
        />
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="p-10 flex flex-col justify-center max-md:p-6 min-w-0">
              <div className="order-1 flex items-center justify-between gap-3 mb-3">
                <span className="text-xs text-[#e67e22] font-bold uppercase tracking-wider">
                  {product.categories?.name || 'Мебель'}
                </span>
                {product.sku && (
                  <span className="text-xs text-[#95a5a6]">Артикул: {product.sku}</span>
                )}
              </div>
              ...
              <div className="order-5 mb-8 text-sm">
                ...
                <div className="grid grid-cols-[100px_1fr] gap-3 items-start py-3 border-t border-b border-[#e5e7eb]">
                  <span className="text-[#64748b] font-medium">Наличие</span>
                  <span
                    className={`justify-self-end text-right font-semibold ${
                      product.in_stock ? 'text-[#27ae60]' : 'text-[#e74c3c]'
                    }`}
                  >
              ...
              {/* На мобиле кнопка "Связаться" уже есть в фиксированной панели
                  внизу экрана, а "Избранное" — поверх фото товара, поэтому
                  этот ряд показываем только на десктопе. */}
              <div className="order-6 hidden md:flex items-center gap-3">
                <ContactSellerButton className="flex-1 md:flex-1 w-full md:w-full" />
                <FavoriteButton productId={product.id} className="w-14 h-14 shrink-0 border border-[#e5e7eb]" />
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Кнопка "Связаться" зафиксирована внизу экрана — только на мобиле:
          на десктопе кнопка уже видна в основном блоке товара (order-6). */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-[90] bg-white/95 backdrop-blur border-t border-[#eee] shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <div className="max-w-[1400px] mx-auto px-5 py-3">
          <ContactSellerButton className="w-full" />
        </div>
      </div>
    </>
  );
