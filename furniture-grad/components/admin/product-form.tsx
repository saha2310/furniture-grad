...
export function ProductForm({
  ...
  return (
    <form action={action} className="bg-white p-6 rounded-2xl shadow mb-8 grid grid-cols-2 gap-4 max-md:grid-cols-1">
      {product && (
        <>
      ...
      <Input name="price" type="number" placeholder="Цена" defaultValue={product?.price} required />
      <Input name="old_price" type="number" placeholder="Старая цена (если есть скидка)" defaultValue={product?.old_price ?? ''} />

      <select
        name="category_id"
        defaultValue={product?.category_id ?? ''}
        className="w-full px-3 py-2 border-2 border-[#ddd] rounded-lg"
      >
      ...
      <textarea
        ...
        className="col-span-2 w-full px-3 py-2 border-2 border-[#ddd] rounded-lg max-md:col-span-1"
        rows={3}
      />

      <Button type="submit" className="col-span-2 max-md:col-span-1">
