...
export function ProductForm({
  ...
  return (
    <form action={handleSubmit} className="bg-white p-6 rounded-2xl shadow mb-8 grid grid-cols-2 gap-4 max-md:grid-cols-1">
      ...
      <Input name="sku" placeholder="Артикул, например ART-1234" defaultValue={product?.sku ?? ''} />
      ...
      <div className="flex gap-4 items-center">
        ...
        <label className="flex items-center gap-2">
          <input type="checkbox" name="in_stock" defaultChecked={product?.in_stock ?? true} /> В наличии
        </label>
      </div>
