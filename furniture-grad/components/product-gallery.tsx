...
export function ProductGallery({
  ...
  if (images.length === 0) {
    return (
      <div className="h-[500px] max-md:aspect-square flex flex-col">
  ...
  const arrows = images.length > 1 && (
    <>
      <button
        ...
        className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white/80 hover:bg-white flex items-center justify-center text-xl text-[#2c3e50] shadow-md transition-colors"
      >
        ‹
      </button>
      <button
        ...
        className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white/80 hover:bg-white flex items-center justify-center text-xl text-[#2c3e50] shadow-md transition-colors"
      >
        ›
      </button>
    </>
  ...
  return (
    <div className="h-[500px] max-md:aspect-square flex flex-col">
      <div
        className="relative flex-1 overflow-hidden bg-[#f0f0f0] cursor-zoom-in"
      ...
      {lightboxOpen && (
        ...
        >
          <button
            ...
            onClick={() => setLightboxOpen(false)}
            aria-label="Закрыть"
            className="absolute top-4 right-4 text-4xl text-white/90 hover:text-white transition-colors"
          >
            ×
          </button>
          <div
