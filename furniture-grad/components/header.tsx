...
export function Header({ shopName }: { shopName: string }) {
  ...
  const { productId } = useHeaderFavorite();
  ...
  return (
    <header className="bg-gradient-to-r from-[#2c3e50] to-[#1a252f] text-white sticky top-0 z-[100] shadow-[0_2px_20px_rgba(0,0,0,0.3)]">
      <div className="px-10 py-5 flex justify-between items-center max-md:px-4 max-md:py-4">
        ...
        <div className="flex items-center gap-3">
          <Link href="/favorites" className="hidden max-md:relative max-md:flex w-9 h-9 items-center justify-center text-xl shrink-0">
            ♥
            {ids.length > 0 && (
          ...
          {productId ? (
            <FavoriteButton productId={productId} className="w-11 h-11 max-md:w-9 max-md:h-9 shrink-0" />
          ) : (
            <button
              onClick={open}
