...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.08)] transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_12px_40px_rgba(0,0,0,0.15)] group">
      ...
      <div className="p-5 bg-[#f8f9fa] rounded-b-2xl">
        ...
        <Link href={`/catalog/${product.slug}`}>
          <h3 className="text-lg font-bold text-[#2c3e50] mb-2 leading-tight hover:text-[#e67e22] transition-colors break-words">
            {product.name}
          </h3>
        </Link>
        <p className="text-sm text-[#7f8c8d] mb-4 line-clamp-2 break-words">{product.description}</p>
