...
async function checkDailyLimit(): Promise<{ allowed: boolean; current: number }> {
  const supabase = await createClient();
  const today = new Date().toISOString().slice(0, 10);
...
async function incrementDailyUsage(current: number): Promise<void> {
  const supabase = await createClient();
  const today = new Date().toISOString().slice(0, 10);
...
export async function processProductImage(formData: FormData): Promise<AiImageResult> {
  ...
  try {
    ...
    await incrementDailyUsage(limit.current);
