-- Счётчик обработок фото по дням — нужен, чтобы ограничить дневной лимит
-- вызовов внешнего API обработки изображений (защита от лишних трат).
...
-- Публичное чтение (нужно, чтобы UI мог показать "осталось N обработок сегодня").
