...
const ToastContext = createContext<{
  // Возвращает true/false, чтобы вызывающий код (например, оптимистично
  // скрытая строка в админке) мог откатить UI назад, если запрос не удался.
  runWithToast: (message: string, action: () => Promise<void>) => Promise<boolean>;
} | null>(null);
